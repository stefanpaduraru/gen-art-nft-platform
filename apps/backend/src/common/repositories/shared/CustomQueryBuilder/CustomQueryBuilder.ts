...
export class CustomQueryBuilder<Entity, ResultType> {
  ...
  public getManyAndCount(includeDeleted = false) {
    ...
    if (this.query.expressionMap.wheres.length === 0) {
      return this.query
        .where(`${this.alias}.isDeleted=0`)
        .getManyAndCount();
    }

    return this.query
      .andWhere(`${this.alias}.isDeleted=0`)
      .getManyAndCount();
  }
  ...
  public async getWithComputedFieldsOne(
    ...
    return this.mapWithComputedField(
      entity,
      computedFieldMappers,
      raw,
    );
  }
  ...
  public getSum(columnAlias: string, resultAlias = 'sum') {
    return this.query
      .select(`SUM(${columnAlias})`, resultAlias)
      .getRawOne();
  }
  ...
  public andFilter(filterParam: FilterParamsType) {
    this.query = this.query.andWhere(
      filterParam.getQuery(this.alias),
    );
  ...
  private mapWithComputedField(
    ...
    const rowData = raw.find(
      (rowDataPacket) =>
        rowDataPacket[`${this.alias}_id`] === entity.id,
    );
